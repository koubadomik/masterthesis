...
Real world use cases provide often more complex datasets than just fixed dimension matrices or images. As we mentioned malware analysis data are often stored as JSON files and modelling of such data is our goal. Those files could be formally seen as tree-structured inputs. In following section we describe two very simple approaches of such data classification (mentioned in \cite{Mandlik2020}) and two more complex. The motivation of more complex techniques is that the data the structure itself keeps also some information and it is worth to try to model it.

\subsection{Rules}
An example of rule might be that if we observe specific value in specific key part in the document, e.g. \ \{"api_calls": "DeleteFileA"\}, we classify this document to specific class, e.g. \ \emph{dropper}. The logic might be more complex, we can count some score aggregating more information about current document and based on its value we might classify. As example of such rule-based approach we can see malware signatures mentioned in previous chapter as their implementation deterministically check some property of original \emph{JSON} report and perform binary classification (\emph{true} - signature is added to the report, \emph{false} - signature is not added to the report).

\subsection{Flattening}
This approach deals with the data structure itself which have not fixed dimensions. Each document might contain different number of keys in different order and size of arrays may vary. Using flattening we find mapping/procedure which allow vectorization of each document. Target learning algorithm is used on the flattened dataset. This might be the background of various methods.

\emph{Graph neural networks}
This kind of neural network was introduced in \cite{Scarselli2009}. \todo{describe or skip}
