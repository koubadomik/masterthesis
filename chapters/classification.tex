...
Examples of discriminative classification learning algorithms are --- Support vector machine, Decision trees, Logistic regression, neural nets.
...
% We want the probability to be the maximum attainable, and $\log$ of value between $0$ and $1$ is negative and larger as we are getting closer to $0$. 
...
This loss was introduced in \cite{Gentile1998} and its well-known usage we can find in the Support vector machine algorithm. The formula could be seen in the figure \eqref{eq:hinge}, where $y\in\mathcal{Y}=\{\pm1\}$ denotes truth label and $\hat{y}$ classifier's score.
...
% For example, \emph{accuracy} used in this article \cite{Ghanaei2016} may make sense because authors are interested just in the positive examples. 
...
% Our model uses \emph{neural net}. That is why we want to describe it more comprehensively than other models.
\begin{figure}
    \centering
...
% \emph{Gradient descent} guarantees stationary point convergence, which could be a \emph{global/local minimum} or a \emph{saddle point}. 
% However, applications lead us to conclude that this model may perform well in the right hand.
...
The natural structure of \emph{JSON} document is a tree which is a less general example of a graph. Thus in theory, GNNs can be used for classification of JSONs, yet the approach might be unnecessary computationaly expensive \cite{Pevny2020}.

\subsection{HMill framework}
In our experiments, we aim specifically at the hierarchical multiple instance learning (\emph{HMill}) framework defined in \cite{Mandlik2020} because it was specifically for tree-structured data and it uses the structure of the input for the model optimization. The authors proposed a use case of \emph{JSON} modelling with good results, and we would like to demonstrate the framework's performance on more complex data. In comparison to GNNs approach, \emph{HMill} model has better scalability and it is computationally efficient, since single pass over the data is sufficient (unlike in GNN where you need multiple passes) \cite{Mandlik2020}. Authors adopted and proved the universal approximation theorem \cite{Hornik1991} to the \emph{HMill} situation which shows that \emph{HMill} can approximate any continuous (measurable) function from the space of \emph{JSON} documents to $R^n$. With respect to mentioned facts, we believe HMIL is more suited for the problem.
