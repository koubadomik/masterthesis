...
This approach is based on the high absolute value of gradients for the most significant parts of the input. That is adopted even in the case of a saliency map of image processing neural net. The crucial idea is to compute the gradient (\ref{eq:grad}) of the model with respect to the embedding of a particular subtree $c$ of a particular sample $d$. Alternatively, we want to examine how much a small change in the embedding of a specific subtree influences the model's prediction. Computation of the gradient ranking $r$ of a particular subtree follows: \ref{eq:grad2}, where $G(c)_i$ denotes i-th item of the vector $G(c)$.

\begin{equation} \label{eq:grad}
    p(c)=\frac{\partial f(h(d))}{\partial h(c)} \in \mathbb{R}^m
\end{equation}

\begin{equation} \label{eq:grad2}
    r(c)=|\sum_{i=1}^{m}G(c)_i|
\end{equation}
